...
public class MeshEx {
	...
	private ShortBuffer m_DrawListBuffer = null; // Holds index values into the
													// vertex buffer that
													// indicate
	...
	private int m_MeshVerticesDataStrideBytes = m_CoordsPerVertex
			* FLOAT_SIZE_BYTES;
	...
	void SetUpMeshArrays(int PosHandle, int TexHandle, int NormalHandle) {
		...
		if (m_MeshHasNormals) {
			...
			GLES20.glVertexAttribPointer(NormalHandle, 3, GLES20.GL_FLOAT,
					false, m_MeshVerticesDataStrideBytes, m_VertexBuffer);
	...
	public static void CheckGLError(String glOperation) {
		...
		while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
			Log.e("ERROR IN MESHEX", glOperation
					+ " IN CHECKGLERROR() : glError - " + error);
