...
public class HUD {
	...
	HUD(Context iContext) {
		...
		for (int i = 0; i < MAX_HUDITEMS; i++) {
			m_HUDItems[i] = new HUDItem(ItemName, NumericalValue,
					ScreenPosition, CharacterSet, Icon, HUDImage);
	...
	int FindHUDItem(String ID) {
		int Slot = -1;
		for (int i = 0; i < MAX_HUDITEMS; i++) {
			if ((m_HUDItems[i].GetName() == ID) && (m_HUDItems[i].IsValid())) {
				Slot = i;
			}
		}
	...
	void UpdateHUDItem(Camera Cam, HUDItem Item) {
		...
		Vector3 CamPos = new Vector3(Cam.GetCameraEye().x,
				Cam.GetCameraEye().y, Cam.GetCameraEye().z);
		Vector3 CameraForward = Cam.GetOrientation().GetForwardWorldCoords();
		Vector3 CameraUp = Cam.GetOrientation().GetUpWorldCoords();
		Vector3 CameraRight = Cam.GetOrientation().GetRightWorldCoords(); // Vector3.CrossProduct(CameraForward,
																			// CameraUp);

		// Local Camera Offsets
		Vector3 CamHorizontalOffset = Vector3.Multiply(PositionLocal.x,
				CameraRight);
		...
		// Put images from icon and numerical data onto the composite hud
		// texture
		...
		if (Item.IsDirty()) {
			...
			if (Icon != null) {
				...
				HUDCompositeTexture.CopySubTextureToTexture(0, 0, 0,
						HealthBitmap);
			}
		...
		HUDComposite.m_Orientation.GetPosition().Set(PositionWorld.x,
				PositionWorld.y, PositionWorld.z);
