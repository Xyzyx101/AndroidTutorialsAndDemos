...
public class BillBoardCharacterSet {
	...
	// Use Settext() to set the text you want to display then use
	// RenderToBillBoard to put this text on
	// the input BillBoard object's texture

	// Total Character Set
	static int MAX_CHARACTERS = 50;
	private int m_NumberCharacters = 0; // Number characters in the character
										// set
	...
	boolean AddToCharacterSet(BillBoardFont Character) {
		if (m_NumberCharacters < MAX_CHARACTERS) {
			...
			Log.e("BILLBOARD CHARACTER SET",
					"NOT ENOUGH ROOM TO ADD ANOTHER CHARACTER TO CHARACTER SET");
			return false;
		}
	}
	...
	void SetText(char[] Text) {
		...
		TextStr = TextStr.toLowerCase(Locale.ENGLISH);
	...
	void DrawFontToComposite(BillBoardFont Obj, int X, int Y,
			BillBoard Composite) {
		...
		if (XEndTexture >= BitmapDestWidth) {
			Log.e("BillBoardCharacterSet::DrawFontToComposite",
					"ERROR Overwriting Dest Texture, Last X Position To Write = "
							+ XEndTexture + ", Max Destination Width = "
							+ BitmapDestWidth);
		} else {
	...
	void RenderToBillBoard(BillBoard Composite, int XOffset, int YOffset) {
		...
		for (int i = 0; i < Length; i++) {
			...
			if (Character != null) {
				// Draw this font to the composite by copying the bitmap image
				// data
				Texture Tex = Character.GetTexture(0);
				Bitmap Image = Tex.GetTextureBitMap();
				...
				DrawFontToComposite(Character, XCompositeOffset, YOffset,
						Composite);
			}
		}
	}
