...
public class PolyParticleEx extends Object3d {
	/*
	 * // With Textures and Normals static float[] PolyParticleVertices = { //
	 * Triangle Shape // Left Side u v nx, ny, nz -0.5f, -0.5f, -0.5f, 0, 1, -1,
	 * -1, -1, // v0 = left, bottom, back -0.5f, -0.5f, 0.5f, 1, 1, -1, -1, 1,
	 * // v1 = left, bottom, front 0.0f, 0.6f, 0.0f, 0.5f, 0, 0, 1, 0, // v2 =
	 * top point };
	 */
	...
	private boolean m_Locked; // true if set to launch or in use, false if
								// available for use
	...
	void Destroy() {
		...
		m_Locked = false; // Particle is now free to be used again by the
							// Particle Manager.
	...
	void Reset() {
		m_TimeStamp = 0;
		m_Locked = false; // true if set to launch or in use, false if available
							// for use
		m_Active = false;
	...
	void LockParticle(float Force, Vector3 DirectionNormalized, long CurrentTime) {
		...
		Vector3 FVector = new Vector3(DirectionNormalized.x,
				DirectionNormalized.y, DirectionNormalized.z);
	...
	void FadeColor(Vector3 ColorIn) {
		...
		// Adjust Brightness Level Down from full brightness = 1 to no
		// brightness = 0;
