...
public class Driver {
	private VehicleCommand m_CurrentOrder = null; // Order to be executed in the
													// FSM
	private VehicleCommand m_LastOrder = null;
	private AIVehicleCommand m_CommandExecuting = null; // Command that is
														// currently being
														// executed in the
	...
	private float m_TurnArea = 2.0f; // Area near waypoint where ai vehicle
										// slows down for turning toward next
										// waypoint
	private Vector3 m_WayPoint = new Vector3(0, 0, 0); // Holds Current Waypoint
	private float m_WayPointRadius = 1.0f; // Radius of CheckPoint/Evasion
											// WayPoint
	...
	Driver(Tank Vehicle) {
		...
		StateTankSteerWayPoint SteerWayPoint = new StateTankSteerWayPoint(
				FSM_StatesTank.FSM_STATE_STEER_WAYPOINT, this);
		StateTankProcessCommand ProcessCommand = new StateTankProcessCommand(
				FSM_StatesTank.FSM_STATE_PROCESS_COMMAND, this);
	...
	void SaveDriverState(String Handle) {
		SharedPreferences settings = m_AITank.GetMainBody().GetContext()
				.getSharedPreferences(Handle, 0);
	...
	VehicleCommand GetCurrentOrder() {
		return m_CurrentOrder;
	}

	VehicleCommand GetLastOrder() {
		return m_LastOrder;
	}

	AIVehicleCommand GetCommandExecuting() {
		return m_CommandExecuting;
	}

	Steering GetTurretSteering() {
		return m_TurretSteering;
	}
	...
	void Update() {
		...
		// Log.e("PILOT","PILOT MACHINE STATE = " +
		// m_FiniteStateMachine.GetCurrentState());
	}
}
