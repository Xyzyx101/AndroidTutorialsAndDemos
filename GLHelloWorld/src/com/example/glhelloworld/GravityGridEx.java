...
public class GravityGridEx {
	...
	private float[] m_MassSpotLightColor = new float[MAX_MASSES * 3]; // 3 r,g,b
	...
	GravityGridEx(
		...
		for (float z = 0; z < GridSizeZ; z++) {
			for (float x = 0; x < GridSizeX; x++) {
				...
				if (index >= TotalNumberCoords) {
					Log.e("GRAVITYGRIDEX",
							"Array Out of Bounds ERRROR, Index >= TotalNumberCoords");
				}
		...
		int TotalDrawListEntries = TotalDrawListEntriesX
				+ TotalDrawListEntriesZ;
		...
		for (int z = 0; z < GridSizeZ; z++) {
			...
			for (int x = 0; x < (GridSizeX - 1); x++) {
				if (index >= TotalDrawListEntries) {
					Log.e("GRAVITYGRIDEX",
							"Array Out of Bounds ERRROR- Horizontal, Index >= TotalDrawListEntries");
				}
		...
		for (int z = 0; z < (GridSizeZ - 1); z++) {
			...
			for (int x = 0; x < (GridSizeX); x++) {
				if (index >= TotalDrawListEntries) {
					Log.e("GRAVITYGRIDEX",
							"Array Out of Bounds ERRROR-Vertical, Index >= TotalDrawListEntries");
				}
		...
		m_LineMeshGrid = new MeshEx(m_CoordsPerVertex,
				m_MeshVerticesDataPosOffset, m_MeshVerticesUVOffset,
				m_MeshVerticesNormalOffset, m_Vertices, m_DrawOrder);
	 ...
	 * if(NumberComponents > MAX_MASSES*3) { Log.e("GRAVITY GRID" ,
	 * "SetMassSpotLightColor(), To many values in Color array "); return false;
	 * }
	 * 
	 * for (int i = 0 ; i < NumberComponents; i++) { m_MassSpotLightColor[i]=
	 * Color[i]; }
	 * 
	 * return true; }
	 */
	...
	/*
	 * void SetMasses(int NumberMasses, Object3d[] Masses) { int
	 * MassLocationIndex = 0;
	 * 
	 ...
	 * ClearMasses();
	 * 
	 * for (int i = 0; i < m_NumberMasses; i++) { m_MassValues[i] =
	 * Masses[i].GetObjectPhysics().GetMass();
	 * 
	 * m_MassLocations[MassLocationIndex] =
	 * Masses[i].m_Orientation.GetPosition().x;
	 * m_MassLocations[MassLocationIndex + 1] =
	 * Masses[i].m_Orientation.GetPosition().y;
	 * m_MassLocations[MassLocationIndex + 2] =
	 * Masses[i].m_Orientation.GetPosition().z; MassLocationIndex =
	 * MassLocationIndex + 3;
	 * 
	 * m_MassEffectiveRadius[i] =
	 * Masses[i].GetObjectPhysics().GetMassEffectiveRadius();
	 * 
	 * } }
	 */
	...
	boolean AddMass(Object3d Mass) {
		...
		int MassLocationIndex = MassesIndex * 3; // each mass has 3 components
													// x,y,z
		int SpotLightLocationIndex = MassesIndex * 3; // each spotlight has 3
														// components r,g,b
		...
		m_MassLocations[MassLocationIndex + 1] = Mass.m_Orientation
				.GetPosition().y;
		m_MassLocations[MassLocationIndex + 2] = Mass.m_Orientation
				.GetPosition().z;
		MassLocationIndex = MassLocationIndex + 3;

		// Add the Radius of the Spotlight for the Mass
		m_MassEffectiveRadius[MassesIndex] = Mass.GetObjectPhysics()
	...
	boolean AddMasses(int iNumberMasses, Object3d[] Masses) {
		...
		for (int i = 0; i < iNumberMasses; i++) {
			...
			// Add the x,y,z location of the Mass
			m_MassLocations[MassLocationIndex] = Masses[i].m_Orientation
					.GetPosition().x;
			m_MassLocations[MassLocationIndex + 1] = Masses[i].m_Orientation
					.GetPosition().y;
			m_MassLocations[MassLocationIndex + 2] = Masses[i].m_Orientation
					.GetPosition().z;
			MassLocationIndex = MassLocationIndex + 3;
	...
	void SetUpShader() {
		...
		//m_Shader.SetShaderUniformVariableValue("time",
		//CalcGridHeightOffset());
		m_Shader.SetShaderUniformVariableValueInt("NumberMasses",
				m_NumberMasses);

		m_Shader.SetShaderVariableValueFloatVector1Array("MassValues",
				MAX_MASSES, m_MassValues, 0);
		m_Shader.SetShaderVariableValueFloatVector3Array("MassLocations",
				MAX_MASSES, m_MassLocations, 0);
		m_Shader.SetShaderVariableValueFloatVector1Array("MassEffectiveRadius",
				MAX_MASSES, m_MassEffectiveRadius, 0);

		m_Shader.SetShaderVariableValueFloatVector3Array("SpotLightColor",
				MAX_MASSES, m_MassSpotLightColor, 0);
