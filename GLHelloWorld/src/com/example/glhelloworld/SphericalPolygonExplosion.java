...
public class SphericalPolygonExplosion {
	...
	boolean m_ParticleColorAnimation; // true if Particles change colors during
										// explosion
	...
	// Shader LocalAxisShader
		...
		m_RandomColors = RandomColors; // true if Particles set to have Random
										// colors
		m_ParticleColorAnimation = ColorAnimation;
		m_ExplosionActive = false;
		m_ParticleSize = new Vector3(ParticleSize.x, ParticleSize.y,
				ParticleSize.z);
		...
		for (int i = 0; i < m_NumberParticles; i++) {
			...
			// Create New Particle
			m_Particles[i] = new PolyParticleEx(iContext, iMesh, iMeshEx,
					iTextures, iMaterial, iShader// ,
			// LocalAxisShader
			);
	...
	Vector3 GetRandomParticleVelocity(int ParticleNumber, float MaxVelocity,
			float MinVelocity) {
		Vector3 ExplosionDirection = m_ExplosionDirection[ParticleNumber];
		Vector3 ParticleVelocity = new Vector3(ExplosionDirection.x,
				ExplosionDirection.y, ExplosionDirection.z);

		float RandomVelocityMagnitude = MinVelocity
				+ (MaxVelocity - MinVelocity) * m_RandNumber.nextFloat();
	...
	void StartExplosion(Vector3 Position, float MaxVelocity, float MinVelocity) {
		...
		for (int i = 0; i < m_NumberParticles; i++) {
			...
			m_Particles[i].GetObjectPhysics().SetVelocity(
					GetRandomParticleVelocity(i, MaxVelocity, MinVelocity));
			...
			// Log.d("DEBUG - STARTEXPLOSION ", "Color = " +
			// m_Particles[i].GetColor().GetVectorString());
		}
	}
	...
	void UpdateExplosion() {
		...
		for (int i = 0; i < m_NumberParticles; i++) {
			...
			if (m_Particles[i].GetActiveStatus() == true) {
				...
				// Log.d("DEBUG - UPDATEEXPLOSION ", "Position = " +
				// m_Particles[i].m_Orientation.GetPosition().GetVectorString());
				// Log.d("DEBUG - UPDATEEXPLOSION ", "Color = " +
				// m_Particles[i].GetColor().GetVectorString());
