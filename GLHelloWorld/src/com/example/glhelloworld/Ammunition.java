...
public class Ammunition extends Object3d {
	...
	void Reset() {
		...
		// Reset Emitters
		/*
		 * int NumEmitters = GetNumberPolyEmitters(); ParticlePolyEmitter
		 * Emitter = null;
		 * 
		 * for (int i = 0; i < NumEmitters; i++) { Emitter = GetPolyEmitter(i);
	...
	void Fire(Vector3 Direction, Vector3 AmmoPosition, Vector3 OffSetVelocity) {
		// 1. Set Fire Status to true
		m_FireStatus = true;

		// 2. Set direction and speed of Ammunition
		// Velocity of Ammo
		Vector3 DirectionAmmo = new Vector3(Direction.x, Direction.y,
				Direction.z);
		...
		m_Orientation.GetPosition().Set(AmmoPosition.x, AmmoPosition.y,
				AmmoPosition.z);

		// 3. Set Ammunition Initial World Position
		m_AmmunitionStartPosition.Set(AmmoPosition.x, AmmoPosition.y,
				AmmoPosition.z);
	}

	/*
	 * boolean IsAmmoTrailFinished(int EmitterNumber) { ParticlePolyEmitter
	 * Emitter = null;
	 * 
	 ...
	 * 
	 * if (NumLockedParticles == 0) { return true; } else { return false; } }
	 */

	/*
	 * boolean AllAmmoTrailsFinished() { boolean AllFinished = true; int
	 * NumberEmitters = GetNumberPolyEmitters(); for (int i = 0; i <
	 * NumberEmitters; i++) { boolean result = IsAmmoTrailFinished(i); if
	 * (result == false) { AllFinished = false; } }
	 * 
	 * return AllFinished; }
	 */

	/*
	 * void EmitAmmunitionParticles(float ForceAvg, Vector3 MainDirectionVector,
	 * long CurrentTime, int NumberParticlesToLaunch, int MaxLaunchDelay, int
	 * EmitterNumber) { // Emit A Particle Cluster from Ammunition
	 * ParticlePolyEmitter Emitter = null;
	 * 
	 * Emitter = GetPolyEmitter(EmitterNumber); if (Emitter != null) {
	 * //Emitter.EmitParticleCluster(ForceAvg, MainDirectionVector, CurrentTime,
	 * NumberParticlesToLaunch, MaxLaunchDelay);
	 * Emitter.EmitParticlesAlongVector(NumberParticlesToLaunch, ForceAvg,
	 * MaxLaunchDelay, MainDirectionVector, CurrentTime);
	 * 
	 * } }
	 */
