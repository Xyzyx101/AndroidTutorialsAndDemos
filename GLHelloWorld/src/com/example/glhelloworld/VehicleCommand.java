...
enum AIVehicleObjectsAffected {
	None, WayPoints, PrimaryWeapon, SecondaryWeapon
};
...
public class VehicleCommand {
	...
	void SaveState(String Handle) {
		...
		// Waypoints
		// Log.e("VEHICLECOMMAND" , "Saved m_NumberWayPoints = " +
		// m_NumberWayPoints);
		...
		if (m_TargetObject != null) {
			...
			editor.putFloat(TargetObjXHandle,
					m_TargetObject.m_Orientation.GetPosition().x);

			String TargetObjYHandle = Handle + "TargetObjY";
			editor.putFloat(TargetObjYHandle,
					m_TargetObject.m_Orientation.GetPosition().y);

			String TargetObjZHandle = Handle + "TargetObjZ";
			editor.putFloat(TargetObjZHandle,
					m_TargetObject.m_Orientation.GetPosition().z);
		}
	...
	static AIVehicleCommand MatchCommand(String CommandStr) {
		...
		} else if (CommandStr.equalsIgnoreCase("Patrol")) {
			Command = AIVehicleCommand.Patrol;
		}

		/*
		 * else if (CommandStr.equalsIgnoreCase("AttackTarget")) { Command =
		 * AIVehicleCommand.AttackTarget; } else if
		 * (CommandStr.equalsIgnoreCase("EvasiveManeuversSpiral")) { Command =
		 * AIVehicleCommand.EvasiveManeuversSpiral; }
		 */

		return Command;
	}

	static AIVehicleObjectsAffected MatchObjectsAffected(
			String ObjectsAffectedStr) {
		...
		} else if (ObjectsAffectedStr.equalsIgnoreCase("WayPoints")) {
			ObjectsAffected = AIVehicleObjectsAffected.WayPoints;
		} else if (ObjectsAffectedStr.equalsIgnoreCase("PrimaryWeapon")) {
			ObjectsAffected = AIVehicleObjectsAffected.PrimaryWeapon;
		} else if (ObjectsAffectedStr.equalsIgnoreCase("SecondaryWeapon")) {
	...
	void LoadState(String Handle) {
		...
		// Way Points
		// Log.e("VEHICLECOMMAND" , "Loaded m_NumberWayPoints = " +
		// m_NumberWayPoints);
		...
		m_TargetObject = new Object3d(m_Context, iMesh, iMeshEx, iTextures,
				iMaterial, iShader// ,
		// LocalAxisShader
		);
	...
	AIVehicleCommand GetCommand() {
		return m_Command;
	}

	AIVehicleObjectsAffected GetObjectsAffected() {
		return m_ObjectsAffected;
	}

	int GetNumberObjectsAffected() {
		return m_NumberObjectsAffected;
	}

	float GetDeltaAmount() {
		return m_DeltaAmount;
	}

	float GetDeltaIncrement() {
		return m_DeltaIncrement;
	}
	...
	/*
	 * String GetCommandString() { String result = "";
	 * 
	 * result += "Command = "; result += m_Command;
	 * 
	 * result += ", ObjectsAffected = "; result += m_ObjectsAffected; result +=
	 * ", NumberObjectsAffected = "; result += m_NumberObjectsAffected;
	 * 
	 * result += ",DeltaAmount = "; result += m_DeltaAmount; result +=
	 * ",DeltaIncrement = "; result += m_DeltaIncrement;
	 * 
	 * result += ",MaxValue = "; result += m_MaxValue; result += ",MinValue = ";
	 * result += m_MinValue;
	 * 
	 * result += ",Number WayPoints = "; result += m_NumberWayPoints;
	 * 
	 * result += ", CurrentWayPointNumber = "; result += m_CurrentWayPointIndex;
	 * 
	 * result += ", CurrentWayPoint = "; result +=
	 * m_WayPoints[m_CurrentWayPointIndex].GetVectorString();
	 * 
	 * return result; }
	 */
}
