...
public class Camera {
	...
	// Shader iShader,
		...
		SetCameraProjection(Projleft, Projright, Projbottom, Projtop, Projnear,
				Projfar);
	...
	void SetCameraProjection(float Projleft, float Projright, float Projbottom,
			float Projtop, float Projnear, float Projfar) {
		m_Projleft = Projleft;
		...
		m_Projnear = Projnear;
		m_Projfar = Projfar;
		Matrix.frustumM(m_ProjectionMatrix, 0, m_Projleft, m_Projright,
				m_Projbottom, m_Projtop, m_Projnear, m_Projfar);
	}

	void SetCameraView(Vector3 Eye, Vector3 Center, Vector3 Up) {
		...
		// Create Matrix
		Matrix.setLookAtM(m_ViewMatrix, 0, Eye.x, Eye.y, Eye.z, Center.x,
	...
	void CalculatePosition() {
		m_Eye.Set(m_Orientation.GetPosition().x, m_Orientation.GetPosition().y,
