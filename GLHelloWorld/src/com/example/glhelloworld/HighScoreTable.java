...
public class HighScoreTable {
	...
	// Render top high scores to the Billboard representing the top player
	// scoreboard
	...
	void SortHighScoreTable() {
		...
		// Only keep top 10 and make room for another to be added to end of
		// array
	...
	void ClearHighScoreTable() {
		...
		HighScoreTableTexture
				.CopySubTextureToTexture(0, 0, 0, BackGroundBitmap);
	}
	...
	void CopyHighScoreEntryToHighScoreTable(int Rank, Camera Cam,
			HighScoreEntry Item) {
	...
	void UpdateHighScoreTable(Camera Cam) {
		if (m_Dirty) {
			...
			// For the Top Ranked entries copy these to the HighScore Table
			// BillBoard
			for (int i = 0; i < MAX_RANK; i++) {
				if (m_HighScoreTable[i].IsValid()) {
					CopyHighScoreEntryToHighScoreTable(i + 1, Cam,
							m_HighScoreTable[i]);
				}
			}
