...
public class Texture {
	...
	boolean InitTexture(int ResourceId) {
		/*
		 * Android public static void glGenTextures (int n, int[] textures, int
		 * offset) Returns n currently unused names for texture objects in the
		 * array textures.
		 */
		...
		/*
		 * Android public static void glBindTexture (int target, int texture)
		 */
		...
		/*
		 * Android static void texImage2D(int target, int level, Bitmap bitmap,
		 * int border) A version of texImage2D that determines the
		 * internalFormat and type automatically.
		 */
	...
	void ActivateTexture() {
		...
		if (m_TextureId != 0) {
			...
			Log.e("ERROR - Texture ERROR- m_TextureId = 0",
					"Error in ActivateTexture()! ");
		}
	}
	...
	void SetTextureWRAP_MAG_FILTER(int value) {
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D,
				GLES20.GL_TEXTURE_MAG_FILTER, value);
	}

	void SetTextureWRAP_MIN_FILTER(int value) {
		GLES20.glTexParameteri(GLES20.GL_TEXTURE_2D,
				GLES20.GL_TEXTURE_MIN_FILTER, value);
	}
	...
	void SetTextureEnvMODE(float value) {
		...
		// texture values are to be combined with the color
		// values of the fragment being processed. If pname is
		// GL_TEXTURE_ENV_COLOR, param is an array of four floating-point values
		// representing R, G, B, and A components. These values are used only if
		// the GL_BLEND texture function has been specified
		// as well.
