...
public class Sound {
	...
	void PlaySound() {
		...
		/*
		 * soundID a soundID returned by the load() function leftVolume left
		 * volume value (range = 0.0 to 1.0) rightVolume right volume value
		 * (range = 0.0 to 1.0) priority stream priority (0 = lowest priority)
		 * loop loop mode (0 = no loop, -1 = loop forever) rate playback rate
		 * (1.0 = normal playback, range 0.5 to 2.0)
		 */
		m_SoundPool.play(m_SoundIndex, m_LeftVolume, m_RightVolume, m_Priority,
				m_Loop, m_Rate);
	}
