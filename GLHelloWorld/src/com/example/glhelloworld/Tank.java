...
public class Tank {
	...
	void SaveTankState(String Handle) {
		...
		// m_MainBody.GetContext().getSharedPreferences(Handle, 0);
	...
	void RenderVehicle(Camera Cam, PointLight Light, boolean DebugOn) {
		...
		boolean ShakeCamera = m_MainBody.GetObjectPhysics()
				.GetHitGroundStatus();
		if (ShakeCamera) {
	...
	void ProcessSteering() {
		...
		HorizontalSteeringValues HorizontalTurn = DriverSteering
				.GetHorizontalSteering();
		...
		} else if (Acceleration == SpeedSteeringValues.Deccelerate) {
	...
	void ProcessTurret() {
		...
		HorizontalSteeringValues HorizontalTurn = TurretSteering
				.GetHorizontalSteering();
	...
	void UpdateVehicle() {
		if (m_MainBody.IsVisible()) {
			...
			// Update Right/Left and Up/Down Rotation of Vehicle based on
			// AIPilot's Steering
			ProcessSteering();
		...
		// Update Vehicle Physics, Position, Rotation, and attached emitters and
		// explosions
		...
		if (m_MainBody.IsVisible()) {
			...
			m_Turret.m_Orientation.GetPosition().Set(OffsetPos.x, OffsetPos.y,
					OffsetPos.z);
		}
