...
public class BillBoard extends Cube {
	...
	void SetBillBoardTowardCamera(Camera Cam) {
		...
		Vector3 ForwardVecProj = new Vector3(
				m_Orientation.GetForwardWorldCoords().x, 0,
				m_Orientation.GetForwardWorldCoords().z);

		// 2. Get The BillBoard Position projected on xz plane
		Vector3 BillBoardPositionProj = new Vector3(
				m_Orientation.GetPosition().x, 0, m_Orientation.GetPosition().z);

		// 3. Get Position of Camera on 2d XZ Plane
		Vector3 CameraPositionProj = new Vector3(Cam.GetCameraEye().x, 0,
				Cam.GetCameraEye().z);

		// 4. Calculate Vector from Billboard to Camera
		Vector3 Bill2CameraVecProj = Vector3.Subtract(CameraPositionProj,
		...
		float Theta = (float) Math.acos(ForwardVecProj
				.DotProduct(Bill2CameraVecProj));
		float DegreeTheta = Theta * 180.0f / Physics.PI;

		// 6. Cross Product to form rotation axis
		Vector3 RotAxis = Vector3.CrossProduct(ForwardVecProj,
				Bill2CameraVecProj);
		...
		if ((Math.cos(Theta) < 0.9999) && (Math.cos(Theta) > -0.9999)) {
			...
			// Log.e("ROTATION BILLBOARD ", "ForwardVecProj = " +
			// ForwardVecProj.GetVectorString()+ ", " +
			// "Bill2CameraVecProj = " + Bill2CameraVecProj.GetVectorString() +
			// ", " +
			// "Rotation Axis = " + RotAxis.GetVectorString() + "," +
			// "Theta = " + Theta);
		} else {
			// Log.e( "BILLBOARD", "No Cylindrical Rotation!! , Theta = " +
			// Theta);
		}

		/*
		 * 
		 * // Spherical BillBoarding Vector3 RightProj =
		 * m_Orientation.GetRightWorldCoords(); //RightProj.y = 0;
		 * 
		 * //Log.e("BILLBOARD " , "RIGHT WORLD COORDS = " +
		 * Right.GetVectorString());
		 * 
		 * Vector3 Bill2Camera = new Vector3(Cam.GetCameraEye().x -
		 * m_Orientation.GetPosition().x, Cam.GetCameraEye().y - 0,
		 * Cam.GetCameraEye().z - m_Orientation.GetPosition().z);
		 * Bill2Camera.Normalize();
		 * 
		 * Vector3 Bill2CameraProj = new Vector3(Cam.GetCameraEye().x -
		 * m_Orientation.GetPosition().x, 0, Cam.GetCameraEye().z -
		 * m_Orientation.GetPosition().z); Bill2CameraProj.Normalize();
		 * 
		 * 
		 * Theta = (float)Math.acos(Bill2Camera.DotProduct(Bill2CameraProj));
		 * DegreeTheta = Theta * 180.0f/Physics.PI;
		 * 
		 * // 7. Rotate BillBoard Toward Camera // cos in radians //if
		 * ((angleCosine < 0.99990) && (angleCosine > -0.9999)) // if
		 * (objToCam[1] < 0) // glRotatef(acos(angleCosine)*180/3.14,1,0,0); //
		 * else // glRotatef(acos(angleCosine)*180/3.14,-1,0,0);
		 * 
		 * 
		 * 
		 * if ((Math.cos(Theta) < 0.9999) && (Math.cos(Theta) > -0.9999)) {
		 ...
		 * 
		 * if (Bill2Camera.y > 0) { m_Orientation.AddRotation(Theta); } else {
		 * m_Orientation.AddRotation(-Theta); } //Log.e("ROTATION BILLBOARD ",
		 * "ForwardVecProj = " + ForwardVecProj.GetVectorString()+ ", " + //
		 * "Bill2CameraVecProj = " + Bill2CameraVecProj.GetVectorString() + ", "
		 * + // "Rotation Axis = " + RotAxis.GetVectorString() + "," + //
		 * "Theta = " + Theta); }
		 */
