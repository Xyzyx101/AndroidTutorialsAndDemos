...
public class StateTankSteerWayPoint extends StateTank {
	...
	void Enter() {
		...
		m_TimeIntervalBetweenFiring = (long) GetParent().GetCurrentOrder()
				.GetDeltaIncrement();

		// Tell the Pilot class what command is actually being executed in the
		// FSM
		GetParent().SetCommandExecuting(AIVehicleCommand.Patrol);

		Log.e("STATETANKSTEERWAYPOINT",
				"ENTERING STATE_TANKSTEERWAYPOINT, TANK CURRENTWAYPOINT = "
						+ m_WayPoint.GetVectorString());
	}
	...
	void Exit() {
		...
		// Log.e("STEERINGWAYPOINT",
		// "EXITING STATESTEERWAYPOINT , NEXT WAYPOINT =  " +
		// GetParent().GetWayPoint().GetVectorString());
	}
	...
	void TurnTurretTowardTarget(Vector3 Target) {
		...
		TurretPosition.x = GetParent().GetAIVehicle().GetTurret().m_Orientation
				.GetPosition().x;
		TurretPosition.z = GetParent().GetAIVehicle().GetTurret().m_Orientation
				.GetPosition().z;

		Vector3 WayPointXZPlane = new Vector3(Target.x, 0, Target.z);
		Vector3 TurretToTarget = Vector3.Subtract(WayPointXZPlane,
				TurretPosition);
		...
		Orientation NewO = new Orientation(GetParent().GetAIVehicle()
				.GetTurret().m_Orientation);
		...
		// 5. Set Steering
		if (Theta2 > Theta) {
			GetParent().GetTurretSteering().SetSteeringHorizontal(
					HorizontalSteeringValues.Right, 1);
			// Log.e("STATESTEERING" ,"Target = " + Target.GetVectorString()+
			// ", Turret STEERING RIGHT");
		} else if (Theta2 < Theta) {
			GetParent().GetTurretSteering().SetSteeringHorizontal(
					HorizontalSteeringValues.Left, 1);
			// Log.e("STATESTEERING" , "Target = " + Target.GetVectorString()+
			// " ,Turret STEERING LEFT");
		} else {
			GetParent().GetTurretSteering().SetSteeringHorizontal(
					HorizontalSteeringValues.None, 0);
			// Log.e("STATESTEERING" , "Target = " + Target.GetVectorString()+
			// ", Turret STEERING STRAIGHT");
		}

	}

	void FireTurretWeapon() {
		Vector3 TurretPos = GetParent().GetAIVehicle().GetTurret().m_Orientation
				.GetPosition();
		...
		} else if (m_WeaponType == AIVehicleObjectsAffected.SecondaryWeapon) {
	...
	void SteerVehicleToWayPointHorizontal(Vector3 WayPoint) {
		...
		VehiclePosition.x = GetParent().GetAIVehicle().GetMainBody().m_Orientation
				.GetPosition().x;
		VehiclePosition.z = GetParent().GetAIVehicle().GetMainBody().m_Orientation
				.GetPosition().z;

		Vector3 WayPointXZPlane = new Vector3(WayPoint.x, 0, WayPoint.z);
		Vector3 VehicleToWayPoint = Vector3.Subtract(WayPointXZPlane,
				VehiclePosition);
		...
		// 5. Set Steering
		if (Theta2 > Theta) {
			GetParent().GetAISteering().SetSteeringHorizontal(
					HorizontalSteeringValues.Right, 1);
	...
	void SteerVehicleWaypointSpeed(Vector3 WayPoint) {
		...
		Vector3 VehiclePos = AIVehicle.GetMainBody().m_Orientation
				.GetPosition();
		Vector3 DistanceVecLastWayPoint = Vector3.Subtract(VehiclePos,
				m_LastWayPoint);
		Vector3 DistanceVecCurrentWayPoint = Vector3.Subtract(VehiclePos,
				m_WayPoint);
		...
		if ((DLastWayPoint <= TurnArea) || (DCurrentWayPoint <= TurnArea)) {
			// Decrease speed
			GetParent().GetAISteering().SetSteeringSpeed(
					SpeedSteeringValues.Deccelerate, 0.04f, 0.03f, 0.005f);
			GetParent().GetAISteering().SetTurnDelta(3.0f);
			// Log.e("STATESTEERING" , "STEERING DECCELERATE");
		} else {
			GetParent().GetAISteering().SetSteeringSpeed(
					SpeedSteeringValues.Accelerate, 0.04f, 0.03f, 0.005f);
	...
	FSM_StatesTank CheckTransitions() {
		...
		if (D <= m_WayPointRadius) {
			Log.e("STATE_TANKSTEERWAYPOINT",
					"TANK WayPoint Reached, WayPoint = "
							+ m_WayPoint.GetVectorString());
