...
public class Shader {
	...
	StringBuffer ReadInShader(int ResourceId) {
		...
		InputStream inputStream = m_Context.getResources().openRawResource(
				ResourceId);

		/*
		 * InputStreamReader
		 * 
		 * A class for turning a byte stream into a character stream. Data read
		 * from the source input stream is converted into characters by either a
		 * default or a provided character converter. The default encoding is
		 * taken from the "file.encoding" system property. InputStreamReader
		 * contains a buffer of bytes read from the source stream and converts
		 * these into characters as needed. The buffer size is 8K.
		 * 
		 * InputStreamReader(InputStream in) Constructs a new InputStreamReader
		 * on the InputStream in.
		 */

		/*
		 * Wraps an existing Reader and buffers the input. Expensive interaction
		 * with the underlying reader is minimized, since most (smaller)
		 * requests can be satisfied by accessing the buffer alone. The drawback
		 * is that some extra space is required to hold the buffer and that
		 ...
		 * 
		 * BufferedReader(Reader in) Constructs a new BufferedReader, providing
		...
		try {
			...
			Log.e("ERROR - SHADER READ ERROR",
					"Error in ReadInShader(): " + e.getLocalizedMessage());
		}
	...
	void SetShaderUniformVariableValueInt(String variable, int value) {
		...
		if (loc < 0) {
			...
			// Log.d("SHADER " , "SetShaderUniformVariableValueInt loc value = "
			// + loc);
		}
	...
	void SetShaderUniformVariableValue(String variable, float value) {
		...
		// Log.d("SHADER " , "SetShaderUniformVariableValue loc value = " +
		// loc);
	...
	void SetShaderVariableValueFloatVector1Array(String variable, int count,
	...
	void SetShaderVariableValueFloatVector3Array(String variable, int count,
			float[] value, int offset) {
		int loc = GLES20.glGetUniformLocation(m_ShaderProgram, variable);
	...
	void SetShaderVariableValueFloatMatrix4Array(String variable, int count,
	...
	void SetShaderVariableValueFloatMatrix3Array(String variable, int count,
			boolean transpose, float[] value, int offset) {
		int loc = GLES20.glGetUniformLocation(m_ShaderProgram, variable);
		// public static void glUniformMatrix4fv (int location, int count,
		// boolean transpose, float[] value, int offset)
