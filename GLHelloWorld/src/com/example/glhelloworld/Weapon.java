...
public class Weapon extends Object3d {
	...
	/*
	 * 
	 * void SetForceAvgParticle(float value) { m_ForceAvgParticle = value; }
	 * 
	 * void SetNumberParticlesToLaunch(int value) { m_NumberParticlesToLaunch =
	 * value; }
	 * 
	 * void SetMaxLaunchDelay(int value) { m_MaxLaunchDelay = value; }
	 */
	...
	boolean Fire(Vector3 Direction, Vector3 WeaponPosition) {
		...
		if (AmmoSlot >= 0) {
			...
			/*
			 * Vector3 MainDirectionVector = new Vector3(-Direction.x,
			 * -Direction.y, -Direction.z); int NumberEmitters =
			 * m_WeaponClip[AmmoSlot].GetNumberPolyEmitters(); for (int i = 0; i
			 * < NumberEmitters; i++) {
			 * m_WeaponClip[AmmoSlot].EmitAmmunitionParticles
			 * (m_ForceAvgParticle, MainDirectionVector, CurrentTime,
			 * m_NumberParticlesToLaunch, m_MaxLaunchDelay, i); }
			 */
	...
	Object3d CheckAmmoCollision(Object3d obj) {
		...
		for (int i = 0; i < MAX_DEFAULTAMMO; i++) {
			// If Ammunition is fired then Update Ammunition and Emit More
			// AmmoDust Trail particles
			if (m_WeaponClip[i].IsFired() == true) {
				// Check Collision
				Physics.CollisionStatus result = m_WeaponClip[i]
						.CheckCollision(obj);
	...
	/*
	 * int GetActiveAmmo(Object3d[] ActiveAmmo) { // Put all active fired
	 * ammunition in ActiveAmmo array // and return the number of fired
	 * ammunition int AmmoNumber = 0; for (int i = 0; i < MAX_DEFAULTAMMO; i++)
	 * { if (m_WeaponClip[i].IsFired() == true) { ActiveAmmo[AmmoNumber] =
	 * m_WeaponClip[i]; AmmoNumber++; } } return AmmoNumber; }
	 */

	/*
	 * int GetActiveAmmo(Object3d[] ActiveAmmo, float[] SpotLights) { // Put all
	 * active fired ammunition in ActiveAmmo array // and return the number of
	 * fired ammunition int AmmoNumber = 0; int SpotLightIndex = 0; for (int i =
	 * 0; i < MAX_DEFAULTAMMO; i++) { if (m_WeaponClip[i].IsFired() == true) {
	 * ActiveAmmo[AmmoNumber] = m_WeaponClip[i];
	 * 
	 * 
	 * float[] Color = m_WeaponClip[i].GetGridSpotLightColor();
	 * SpotLights[SpotLightIndex] = Color[0]; SpotLights[SpotLightIndex+1]=
	 * Color[1]; SpotLights[SpotLightIndex+2]= Color[2];
	 * 
	 * SpotLightIndex = SpotLightIndex + 3;
	 * 
	 * AmmoNumber++; } } return AmmoNumber; }
	 */

	/*
	 * int GetActiveAmmo(int StartIndex, Object3d[] ActiveAmmo, float[]
	 * SpotLights) { // Put all active fired ammunition in ActiveAmmo array //
	 * and return the number of fired ammunition int AmmoNumber = StartIndex;
	 * int SpotLightIndex = StartIndex * 3; // Account for r,g,b components for
	 * (int i = 0; i < MAX_DEFAULTAMMO; i++) { if (m_WeaponClip[i].IsFired() ==
	 * true) { ActiveAmmo[AmmoNumber] = m_WeaponClip[i];
	 * 
	 * 
	 * float[] Color = m_WeaponClip[i].GetGridSpotLightColor();
	 * SpotLights[SpotLightIndex] = Color[0]; SpotLights[SpotLightIndex+1]=
	 ...
	 * SpotLightIndex = SpotLightIndex + 3;
	 * 
	 * AmmoNumber++; } } return (AmmoNumber - StartIndex); }
	 */
	...
	void UpdateWeapon() {
		...
		for (int i = 0; i < MAX_DEFAULTAMMO; i++) {
			...
			if (m_WeaponClip[i].IsFired() == true) {
				...
				Vector3 AmmoCurrentPos = m_WeaponClip[i].m_Orientation
						.GetPosition();
				Vector3 AmmoInitPos = m_WeaponClip[i]
						.GetAmmunitionStartPosition();
				Vector3 DistanceVector = Vector3.Subtract(AmmoCurrentPos,
						AmmoInitPos);
